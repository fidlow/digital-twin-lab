...
type SeriesItem = [string, string, string, number[], string];
...
type RiskResult = [string, string, string[], string[]];
...
const rnd = (x: number, d: number = 2) => Number(x.toFixed(d));
const noise = (a: number) => (Math.random() - 0.5) * a;
const time = () => new Date().toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit", second: "2-digit" });

function point(k: number, mode: string, prev?: DataPoint): DataPoint {
  const q = DT / 900;
  const n = Math.sqrt(q);
...
function risk(m: DataPoint): RiskResult {
  const bad: string[] = [];
  const warn: string[] = [];
...
function evidence(m: DataPoint): EvidenceResult {
...
function advice(mode: string, r: RiskResult): [string, string, string] {
