...
export const EXPLANATIONS: Record<string, Explanation> = {
  ...
  i: {
    ...
    body: "Базовый ток умножается на ползунок нагревателя u_h (0,6–1,6). В режиме «Перегрев» к нему каждый тик прибавляется ΔI_реж ≈ 0,04 А (накапливается) — это симулирует заклинивший термостат: установка перестала ограничивать нагрев, и ток медленно растёт со временем сам по себе.",
    constants: [
  ...
  w: {
    ...
    body: "Простой закон Ома: напряжение постоянное, мощность линейна по току. В режиме «Перегрев» рост W — это следствие медленно растущего тока (см. «Ток нагрузки»), а не действий оператора.",
    constants: [
